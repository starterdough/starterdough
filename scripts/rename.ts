 ...
 *
 * It rewrites four tokens across the tracked text files: the product name, the lowercase slug (npm
 ...
 * workspace scope. The current values are read from the repo rather than hardcoded, so a second run
 * with different arguments works.
 *
 * What it deliberately does not do is printed as a residue checklist at the end: anything where a
 * text substitution would look right and leave you broken — Docker volumes, an existing dev
 * database, the untracked `.env`, lockfiles, and the seller's GitHub coordinates.
 */
...
/**
 * The seller's own repository. Left verbatim everywhere it appears: rewriting it produces a
 * plausible URL that does not exist, and `UPGRADING.md` needs it to stay pointed at the kit.
 *
 * Each edition is published from its own repository, so the value differs between them, and there
 * is nothing in a checkout to derive it from — hence a marker rather than a lookup.
 */
const KIT_REPO = 'starterdough/starterdough';

/**
 * Files that describe the kit rather than your product. Renaming the licence you were granted, its
 * third-party notices, the kit's release history or its upgrade instructions makes them wrong.
 */
 ...
 * slug matching inside a longer word while still matching `<slug>_`, `<slug>-api`, `<slug>:` and
 * `<slug>` at end of line. It was written for the original slug `director`, which is a prefix of
 * `directory`/`directories` — 60 occurrences here that a naive replace would have corrupted — and
 * it still earns its place for any buyer whose slug is a prefix of an English word.
 */
...
/**
 * Splitting on the kit's own repository slug and rewriting only the segments around it is what keeps
 * that string verbatim — no placeholder that could survive into a written file.
 */
...
const kitRepoSites = await (async () => {
	const grep = Bun.spawn(['git', 'grep', '-n', '--fixed-strings', KIT_REPO], {
		cwd: repoRoot,
		stdout: 'pipe',
		stderr: 'ignore',
	});
...
5. The kit's own GitHub coordinates were left as they are, on purpose — point them at your fork:
${kitRepoSites.map((line) => `     ${line}`).join('\n') || '     (none found)'}
