 ...
 *
 * No dependencies: it reads every `package.json` under a `node_modules` and, when a Python
 * virtualenv exists, the `*.dist-info/METADATA` files `uv sync` wrote. Install first; packages that
 * are not installed are not reported. Dev dependencies are included because container images ship
 * some of them.
 */
...
/**
 * Packages whose non-permissive licence already has a notice in `THIRD-PARTY.md`. Still printed,
 * but they do not fail `--strict`. Prefixes, because each ships one package per platform. Delete
 * an entry when you drop the dependency; re-read the notice when its licence expression changes.
 */
...
/**
 * Some packages ship a LICENSE file and no `license` field (native sidecars, mostly). Reading the
 * file's opening line is a heuristic, so the result is labelled as such.
 */
 ...
 * not parseable into known ids (`SEE LICENSE IN …`, `UNLICENSED`, an empty field) needs review.
 */
...
async function collectPython(into: Map<string, Entry>): Promise<void> {
	...
	for (const sitePackages of candidates) {
		...
		for (const name of names) {
			...
			// The service itself is installed editable into its own venv; it is not a dependency.
