...
bun run build          # dist/ — also generates the OG images, sitemap, RSS and robots.txt
bun run preview
```
...
| --- | --- | --- |
| `SITE_URL` | `http://localhost:4321` for `dev`/`check`; **required** for `build` | Canonical origin (build time): `<link rel="canonical">`, sitemap, RSS, absolute `og:image` URLs and the `Sitemap:` line in `robots.txt`. `astro build` refuses an unset value and refuses an `example.com` placeholder — either ships a site whose every absolute URL points at someone else's host, with no error to notice |
...
| `PUBLIC_REPO_URL` | empty | Optional. Your public repository: the footer link and its label, "View on GitHub" on the landing page, the changelog's commit history, the contact page's "found a bug". Unset, none of them render — the kit ships no repository URL of its own |
| `PUBLIC_CONTACT_EMAIL` | empty | Optional. Shown to visitors who cannot use the contact form (no JavaScript). Not where the form delivers — that is the API's `CONTACT_EMAIL`, which never reaches the browser |
...
is not an env var: it comes from the root `package.json`, the one place a version is written down.
...
templates with `[Company name]`, `[Contact email]`, … placeholders and a visible "review with
counsel" note — replace them before publishing. The privacy template's sub-processor table lists
...
frontmatter — there is no second date to keep in step.

## Open Graph images

Generated at build time by `astro-og-canvas` (CanvasKit; no runtime, so the output stays
deployable as static assets): one image per static page, blog post and legal document at
`/og/<route>.png`. `Base.astro` derives the key from the current pathname (`/` → `index`,
`/blog/x/` → `blog/x`), so every page's `og:image` exists; pass `ogImage` to override (the 404
page reuses the home image). Fonts (Inter 400/700) are downloaded from fontsource during the
build and cached in `node_modules/.astro-og-canvas` — the build needs network access the first
time.

## Contact form

`/contact` is a plain HTML form with **no `action`**, enhanced by a small script that POSTs JSON to
`${PUBLIC_API_URL}/api/v1/contact` (`contact.send` in the API contract). The API answers JSON and
this is a static site, so a native form POST would land the visitor on a page of raw JSON: instead
the submit button ships `disabled`, the script enables it, and a `<noscript>` block offers
`PUBLIC_CONTACT_EMAIL` (or the repository) as another way through.

- request `{ "email": string, "name"?: string, "message": string }` (message 10–2000 chars) plus
  a hidden honeypot field `website` that the API drops silently when filled;
- `200 { ok: true }` → success shown inline; `400` → the API's validation issues are mapped onto
  the fields; `412` → "not configured on this deployment" (the API has an email provider but no
  `CONTACT_EMAIL`); `429` → rate-limited message; network errors → "could not reach the server".

The API's CORS allow-list (`TRUSTED_ORIGINS` in the root `.env`) must include this site's origin
for the browser to be allowed to POST. **Cloudflare Turnstile** is not wired yet: the form has a
comment where the widget goes and the submit handler has the line that would forward the
`cf-turnstile-response` token as `turnstileToken`, for when the API verifies one.
...
the self-hosted stack. Set `SITE_URL`, `PUBLIC_APP_URL`, `PUBLIC_DOCS_URL` and `PUBLIC_API_URL`
for the target environment before building — the build fails without a real `SITE_URL`.
