/**
 * The feature list shared by the landing page (short) and `/features` (long). Every statement is
 * taken from the repository README and `docs/DECISIONS.md`; keep it that way when editing.
 */
...
export const features: Feature[] = [
	{
		...
		details: [
			...
			'Rate limits on the auth routes and on every procedure, trusting a single forwarded IP — what Caddy produces.',
			'Auth is transport-aware, not platform-aware: browsers use an httpOnly cookie, the native shells use a bearer token. The UI code is identical.',
		],
	...
	},
	{
		...
		details: [
			...
			'Forms use sveltekit-superforms in SPA mode with Zod 4 — no form actions, because the API is the backend on every target. Schemas come from the contract where a procedure exists.',
			...
			'Accessibility is tested: axe (WCAG 2.2 AA) runs on the public pages in the end-to-end suite, and the authenticated pages passed the same checks in both color modes.',
		],
	...
	},
	{
		...
		title: 'Self-hosting',
		summary:
			'Docker Compose with Postgres, the API, the web app and Caddy. Two Caddy modes — public subdomains or a single origin — and Tailscale for anywhere access with no open ports.',
		details: [
			...
			'Tailscale on the host: `tailscale serve` exposes the stack to your devices, `tailscale funnel` makes the same URL public — no open ports, no DNS, TLS handled.',
