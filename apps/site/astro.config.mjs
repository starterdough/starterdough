 ...
 * config — so `SITE_URL` in `.env` (the one variable this file reads) was picked up by nothing, and
 * the site silently built against the fallback. Load it here, anchored to this file rather than the
 * working directory. `process.loadEnvFile` never overwrites a variable that is already set, so an
 * explicit environment value — CI, `Dockerfile.static`, the deploy workflow — still wins, and
 * `.env.local` (loaded first) wins over `.env`, matching Vite's own precedence.
 */
 ...
 * entry and the `robots.txt` `Sitemap:` line. Left unset — or left on the example domain — the
 * build still succeeds and ships a site whose every absolute URL points at somebody else's host,
 * with nothing to notice. So `astro build` refuses both, the way `deploy.yml` refuses the same
 * four variables. `astro dev` and `astro check` keep the localhost fallback.
 *
...
export default defineConfig({
	// Canonical URL — required for sitemaps/RSS/OG tags. Override per environment.
