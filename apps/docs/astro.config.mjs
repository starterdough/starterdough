 ...
 * config — so `SITE_URL` and `DOCS_REPO_URL` in `.env` were picked up by nothing. Load it here,
 * anchored to this file rather than the working directory. `process.loadEnvFile` never overwrites a
 * variable that is already set, so an explicit environment value — CI, `Dockerfile.static`, the
 * deploy workflow — still wins, and `.env.local` (loaded first) wins over `.env`, matching Vite's
 * own precedence.
 */
 ...
 * repository URL of its own in a buyer's docs chrome (a find-and-replace of the product name would
 * not have caught one).
 */
const repoUrl = process.env.DOCS_REPO_URL?.trim().replace(/\/+$/, '') || null;

// SITE_URL is the canonical origin — and, when it carries a path (https://host/docs for the
// single-origin Caddy mode), the base every link and asset is served under.
 ...
 * Left unset — or left on the example domain — the build still succeeds and ships docs whose
 * absolute URLs point at somebody else's host, with nothing to notice. So `astro build` refuses
 * both, the way `deploy.yml` refuses the same four variables. `astro dev` and `astro check` keep
