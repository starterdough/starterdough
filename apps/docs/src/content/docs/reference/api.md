...
- **Browsers** send the session cookie (`credentials: 'include'`). In SvelteKit `load` functions,
  pass SvelteKit's `fetch` so server-side rendering forwards the browser's cookies.
...
version from the **root** `package.json`. Workspace packages are unversioned.
...
| --- | --- | --- | --- |
| `system.health` | `GET /health` | public | none |
| `system.authConfig` | `GET /auth-config` | public. Returns `{ socialProviders, requireEmailVerification }` | none |
...
| `UNAUTHORIZED` | 401 | none | No session (or an expired one) |
| `FORBIDDEN` | 403 | none | Signed in, but not permitted / not a platform admin |
| `NOT_FOUND` | 404 | none | No such row |
| `PRECONDITION_FAILED` | 412 | none | The deployment is missing configuration: the contact form has no recipient address |
...
1. Declare it in `packages/api-contract/src/index.ts` with an explicit `route` (so it is also REST)
 ...
 2. Implement it in `apps/api/src/rpc/` with `requireAuth` or `requireAdmin` from `rpc/base.ts` as
    appropriate.
...
and `/api/v1/*` (300 requests per minute per IP by default). It is a guardrail against runaway
clients, not a defence against determined abuse. Put that at the edge. Both trust `x-forwarded-for`
only as a single value, which is what Caddy produces. CORS allows the origins in `TRUSTED_ORIGINS`
(plus `WEB_URL`) with credentials, and exposes the `set-auth-token` header for bearer clients. Every
...
The per-IP limiter is **in-process**. It is sized for one API container. Running several replicas
behind Caddy needs a shared store first.
