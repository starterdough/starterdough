...
| `bun run licenses` | audit every dependency's licence from the metadata an install already left on disk (`--all` for one line per package, `--strict` to exit 1 on anything not already recorded in `THIRD-PARTY.md` — what CI runs) |
| `bun run rename` | rename the kit to your product across the tracked text files: `--name`, `--slug`, `--identifier`, `--scope`. Dry run unless `--write`; `LICENSE.md`, `THIRD-PARTY.md`, `CHANGELOG.md`, `UPGRADING.md` and the script itself are left verbatim, and it prints a residue checklist of what a text substitution must not touch (Docker volumes, the dev database, `.env`, lockfiles, the kit's own repository URL) |
...
| `bun run db:migrate` | apply `packages/db/drizzle/*.sql`, then verify nothing is still pending and **exit 1** if something is — a branch merge can interleave migration timestamps, which Drizzle's migrator would otherwise skip while printing success |
...
| `bun run admin:create -- --email … [--name …] [--yes]` | create or promote the first platform administrator. The password comes from `ADMIN_PASSWORD` or an interactive prompt, never from the command line, where it would land in shell history and `ps`. It runs through the kit's own auth config — no `bunx`, no network — so it works inside the API container too. `--yes` is required to promote an account that already exists |
...
These are each app's own scripts, not root ones: run them from that directory, or from the root with
`bun run --cwd apps/<app> <script>`.
