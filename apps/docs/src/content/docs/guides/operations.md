...
The password comes from `ADMIN_PASSWORD`, or from an interactive prompt when that is unset — never
...
`NET_BIND_SERVICE`) under a 512-process cap, and Postgres gets `shm_size: 1gb` so parallel scans can
parallelise.

| Profile | Service | What |
| --- | --- | --- |
| `backup` | `backup` | nightly `pg_dump` (+ uploads), S3 copy, heartbeat — `infra/backup` |
| `worker` | `worker` | job worker as its own process (`WORKER_ENABLED=false` on the API). Its container health check is disabled by design: it shares the API image but serves no HTTP, so the image's `HEALTHCHECK` could never pass |
...
```sh
# after enabling the backup profile: prove it works now, do not wait for 02:30 (or set BACKUP_ON_START=true)
...
`starterdough_<stamp>_pre_restore.dump` into the backups directory first, so a restore of the wrong stamp
is itself recoverable (`--no-safety-dump` skips it; safety dumps are never pruned).

Before it touches the database the tool refuses three things, and only two of the refusals have an
override:
...
The dump is **plaintext** — password hashes, sessions, email addresses, every column —
...
outright — any `change-me…` value, and a `DATABASE_URL` whose password is `starterdough`, `postgres` or
