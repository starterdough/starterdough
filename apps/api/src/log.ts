 ...
 * `DrizzleQueryError` whose own properties are the statement and every bound value, so a failing
 * session lookup would log `select … where token = $1` next to the token itself. The rest are the
