 ...
 * `turbo.json` declares as a build input; reading only `.env` used to make the "cannot drift"
 * claim false.
 */
...
// it is never fetched from inside the webview and stays out of `connect-src` — but an installer
// built without it dead-ends those flows, which is why a release build demands it.
...
/** The root `package.json` version is the repo's single source of truth (HANDOFF → "Version SSOT"). */
