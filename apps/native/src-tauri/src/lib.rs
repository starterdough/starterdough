...
/// Release builds used to produce no logs at all, so "it opens and closes again" came with nothing
/// to send. The plugin writes to the OS log directory in both profiles
...
/// where it can be found: the log file, plus a plain-text note in the temp directory — that needs no
