...
# on an active firewall would silently undo a policy the owner changed by hand. This block used to
# skip everything when ufw was already active, which left a box provisioned twice — or provisioned
# after `ufw enable` — with a live firewall and no rule allowing the tailnet or SSH through.
...
# DOMAIN stays empty in BOTH modes. This script cannot know the buyer's domain, and a placeholder
# would have Caddy chase a Let's Encrypt certificate for a domain nobody owns — retrying forever
# while `docker compose ps` reports every container healthy. Empty falls back to `localhost` and
# Caddy's internal CA: the stack serves, the browser warns, and the message printed at the end says
# which row to fill in. (`.env.example` ships it empty for the same reason.)
if [ "$EXPOSE" = public ]; then
	CADDY_BIND=0.0.0.0
