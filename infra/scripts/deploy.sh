...
if [ "${BUILD:-0}" = 1 ]; then
	...
	# (`starterdough/api:<tag>`), so a pull would fail with docker's own "pull access denied" — accurate
	# but not actionable. Say what to set instead. BUILD=1 needs no registry at all.
...
# completed) or fails after the timeout — a broken deploy fails here, loudly.
echo "▸ starting"
