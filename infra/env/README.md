...
The server reads one `.env` at the repository root (`infra/compose.yml` → `env_file`). This folder is
where an **encrypted** copy of it lives so the real values are versioned, reviewable and deployable
from CI without ever being committed in clear text.
...
2. Create a key once: `age-keygen -o ~/.config/sops/age/keys.txt`. Put the printed public key
   (`age1…`) in `/.sops.yaml`. Back the file up — it is the only way to read the encrypted env.
3. Encrypt your filled-in production env:
   ...
   ```
   Names stay in clear text, values are ciphertext. Edit later with `sops infra/env/production.enc.env`.
...
5. By hand on the server: `sops --decrypt --input-type dotenv --output-type dotenv infra/env/production.enc.env > .env`.

Rotate a secret: change it in the encrypted file (`sops …`), commit, push — the next deploy ships it
and restarts the containers that read it. Rotate the age key: add the new recipient to `.sops.yaml`,
...
- **1Password**: keep the env as a Secure Note or use references (`op://vault/item/field`) in a
  template and render it on the server with `op inject -i .env.tpl -o .env` (Service Account token in
...
- **Plain `.env` on the server** (what `infra/scripts/provision.sh` writes): fine for one box you
  ...
  database, because a restore without it is not a restore.
