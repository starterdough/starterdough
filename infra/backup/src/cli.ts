...
async function schedule(config: BackupConfig, log: Logger): Promise<number> {
	...
	const runOnce = async (trigger: 'tick' | 'start' | 'catch-up') => {
		...
		try {
			...
			// In a `finally`: an error thrown outside the inner try — a logger that throws, an
			// `await running` rejected by something added later — used to leave the guard latched and
			// every subsequent tick skipped with "backup still running" for as long as the process lived.
			running = null;
		}
	};
