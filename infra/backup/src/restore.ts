...
/**
 * Refuses the restore while anything else is connected to the target. This used to be a warning,
 * which was the wrong shape twice over: an API that keeps writing during `--clean --if-exists` ends
 * up with rows referencing objects that were dropped underneath it, and `--single-transaction`
 * together with `--clean` cannot take its locks at all while another session holds them — so the
 * restore hangs until `lock_timeout` and then rolls back an hour of work.
 *
