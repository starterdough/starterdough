...
// that, every virtual user shares one bucket. So run this against the API origin directly (port
// 3000, Compose network, dev), not through Caddy: in strict mode (infra/caddy) Caddy replaces the
// client-supplied X-Forwarded-For with the real client address. That is the intended behaviour: load
// tests go to the origin, the edge keeps its limits.
//
 ...
 * and each VU actually runs into its own 300 calls/min budget. Mixing `__ITER` into the address
