...
/**
 * Control characters in stored free text: refused here, at the only edge every caller passes.
 *
 * They are not a display problem. A newline in a name reaches an email subject as a second header,
 * a code point in a `content-disposition` filename throws where the header is built, and either
 * splits a log line. Sanitising at each sink means finding every sink; this is one rule.
 *
